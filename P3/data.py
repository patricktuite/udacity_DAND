...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for tag in element.iter('tag'):
            ...
            node_tags['value'] = clean_value(tag_key, tag_value)
            problems = problem_chars.search(tag_key)
            colons = re.compile(r':')

            if problems:
                continue

            if colons.search(tag_key):
                split_tag_key = colons.split(tag_key)
                node_tags['type'] = split_tag_key[0]
                node_tags['key'] = ":".join(split_tag_key[1:len(split_tag_key)])
            else:
        ...
        for nd in element.iter('nd'):
            ...
            way_nodes_dict['position'] = node_index
            node_index += 1
        ...
        for tag in element.iter('tag'):
            ...
            tag_value = tag.attrib['v']
            tag_key = tag.attrib['k']
            way_tags['value'] = clean_value(tag_key, tag_value)
            ...
            if colons.search(tag_key):
                split_tag_key = colons.split(tag_key)
                way_tags['type'] = split_tag_key[0]
                way_tags['key'] = ":".join(split_tag_key[1:len(split_tag_key)])
            else:
                way_tags['type'] = default_tag_type
                way_tags['key'] = tag_key
